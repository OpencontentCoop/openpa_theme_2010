...
<div class="block-type-lista block-{$block.view} block-lista_tab">
	...
	<div class="ui-tabs">	
		...
		<div class="border-content" id="zone-id-{$block.id}">
			<ul class="ui-tabs-nav">							 
				...
				<li class="{$node.name|slugize()} {if $index|eq(0)}ui-state-active{else}ui-state-default{/if}">											
					<a href={$node.url_alias|ezurl()} title="{$node.name|wash()}"><span class="block-{$block.id}-{$node.name|slugize()}">{$node.name|wash()}</span></a>					
				</li>
				{/foreach}
			</ul>
		</div>
		...
		<div class="tabs-panels">			
			...
			<div id="block-{$block.id}-{$node.name|slugize()}" class="{if $index|gt(0)}no-js-hide {/if}ui-tabs-hide">
